...
class HrlepsoSwarm(HrlepsoBaseSwarm):
    ...
    def __init__(self, n_run, n_part, show, fun, n_dim, pos_max, pos_min, config_dic):
        ...
        model = config_dic.get('model',
                               r"D:\develop\swam\ieeeaccess - 副本 - 副本 - 副本 - 副本\rl\train0\ddpg_actor_episode100.pth")
        ...
        ddpg = DDPG(gym_env, discrete=False, gamma=0, sigma=0.25, actor_units=(16,),
                    critic_units=(8, 16, 32), device='cpu')
    ...
    def run_once(self, action=np.zeros(10)):
        ...
        action_array = to_numpy_array(action)
        # print(f'action:{action}')
        if self.show:
            print(np.mean(np.abs(action_array)))
            print(action_array[:10])
        super(HrlepsoSwarm, self).run_once(action_array)
...
if __name__ == '__main__':
    model = r'''D:\develop\swam\ieeeaccess - 副本 - 副本 - 副本 - 副本\rl\train0\ddpg_actor_episode300.pth'''
